...
public:
    ...
    //*寻找光条中心（单激光条）*/
    void findLightbarCenter(cv::Mat &InputImg, std::vector<cv::Mat> &ImaeCenter, int TYPE = 3);
    //*寻找光条中心（多激光条）*/
    ...
    //*单目模式下计算坐标*/
    void processSingleMonocular(Mat& InputSrc, double _distance, bool isSinglePoint, int sonsorNum);
    //*双目模式下计算坐标*/
...
public slots:
    ...
    void receiveThresholdFromMaindow(int val);
private:
